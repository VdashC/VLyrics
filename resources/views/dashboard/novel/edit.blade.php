...
<div class="col-lg-8">
    ...
    <form method='post' action='/dashboard/novel' class='mb-5' enctype="multipart/form-data">
        ...
        <div class="mb-3">
            <input type="hidden" class="form-control @error('novel_slug') is-invalid @enderror" id="novel_slug"
                name='novel_slug' readonly required value="{{old('novel_slug',$novel->novel_slug)}}">
            @error('novel_slug')
            <div class="invalid-feedback">
                {{$message}}
            </div>
        ...
        <div class="mb-3">
            ...
            <input type="hidden" name="oldImage" value="{{$novel->image}}">
            @if($novel->image)
            <img class='img-preview img-fluid mb-3 col-sm-5 d-block' src="/myfiles/{{$novel->image}}">
            @else
